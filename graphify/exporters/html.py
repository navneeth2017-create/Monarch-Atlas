...
{_hyperedge_script(hyperedges_json)}
    ...
    # Monarch Atlas viewer (default). GRAPHIFY_THEME=classic gives upstream's.
    # Upstream's own tests assert on their viewer, so under pytest the default
    # is classic unless GRAPHIFY_THEME is set explicitly (tests/test_atlas.py does).
    _theme = os.environ.get("GRAPHIFY_THEME") or ("classic" if "PYTEST_CURRENT_TEST" in os.environ else "atlas")
    if _theme != "classic":
        from graphify.exporters import atlas_html
        html = atlas_html.build_document(
            title=title, stats=stats, nodes_json=nodes_json, edges_json=edges_json,
